...
export default function FinanceOverview({ totalIncome, totalExpense, balance }: FinanceOverviewProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      <Card className="space-y-2">
        <p className="text-sm text-[#8a8a8a] font-medium uppercase tracking-wide">Income</p>
        <p className="text-2xl font-bold text-[#6b8cce]">+${totalIncome.toFixed(2)}</p>
      </Card>
      
      <Card className="space-y-2">
        <p className="text-sm text-[#8a8a8a] font-medium uppercase tracking-wide">Expense</p>
        <p className="text-2xl font-bold text-[#ce6b6b]">-${totalExpense.toFixed(2)}</p>
      </Card>
